...
{
    internal class VeldridTextureSub : VeldridTexture
    {
        private readonly VeldridTexture parent;
        private RectangleI bounds;

        public override RectangleI Bounds => bounds;

        public override VeldridTexture Native => parent.Native;
        ...
        public VeldridTextureSub(RectangleI bounds, VeldridTexture parent, WrapMode wrapModeS = WrapMode.None, WrapMode wrapModeT = WrapMode.None)
            : base(wrapModeS, wrapModeT)
        {
            // If Veldrid is not initialized at this point, it means we do not have a graphics backend available
            // and thus will never draw anything. In this case it is fine if the parent texture is null.
            if (Vd.IsInitialized && parent == null)
